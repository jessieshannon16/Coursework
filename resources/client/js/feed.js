function pageLoad(){
    ...
    fetch('/avatarstats/view', {method: 'get'}
        ...
        if (stats.hasOwnProperty('error')) {
            ...
            document.getElementById("stats").innerHTML = `<div>Hunger: ${stats.Hunger}`;

            document.getElementById("tacoButton").addEventListener("click", feed);
...
function feed(event){
    ...
    fetch('/avatarstats/feed', {method: 'get'}
    ).then(response => response.json()
    ).then(stats => {
        if (stats.hasOwnProperty('error')) {
            alert(stats.error);
